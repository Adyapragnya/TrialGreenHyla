...
const getFilteredRoutes = (role) => {
  if (role === "hyla admin") {
    ...
    return allRoutes.filter((route) => ["sign-in" ,"reset-password" , "HYLA", "dashboard", "Geofence" , "alerts", "create-users", "operation-dashboard" , "vessel-tracker", "SalesRadar", "OpsRadar"].includes(route.key));
  } else if (role === "organizational user") {
    // Return only specific routes for Organization User
    return allRoutes.filter((route) => ["sign-in" ,"reset-password" , "HYLA", "dashboard", "Geofence",  "operation-dashboard" , "vessel-tracker" , "SalesRadar", "OpsRadar"].includes(route.key));
  } 
  else if (role === "guest") {
